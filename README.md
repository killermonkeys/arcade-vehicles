...
* `planTurn`/`planAccel` compute the sprite's heading from its velocity (`sprite.vx`/`.vy`), same as `Math.atan2(sprite.vy, sprite.vx)`. While a following sprite is stopped (velocity is exactly `(0, 0)`), there's no velocity to read a heading from, so each sprite instead keeps reporting whichever heading it last had while actually moving (starting from `0`, i.e. facing right, before it's moved at all) - this avoids the turn direction randomly collapsing to "facing right" every time the sprite brakes to a stop.
